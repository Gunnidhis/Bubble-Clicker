...
function makeBubble() {
  let bubble_addition = "";
  for (let i = 1; i <= 140; i++) {
    bubble_addition += `<div style = "background-color:${randomColor()};"class="bubble">${Math.floor(
      Math.random() * 10
    )}</div>`;
  }
  ...
  interval_Id = setInterval(function () {
    if (timer > 0) {
      ...
      document.getElementById(
        "panel_bottom"
      ).innerHTML = `<h1 style ="position:absolute ; top:50% ; text-align: center;" >Game Over <br>Your Score was ${score}</h1>`;
